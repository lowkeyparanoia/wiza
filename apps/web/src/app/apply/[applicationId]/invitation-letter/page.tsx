...
export default function InvitationLetterPage() {
  ...
  function handleFinish() {
    router.push(`/apply/${applicationId}/cover-letter`);
  }

  return (
    <div className="mx-auto max-w-xl px-4 py-4 sm:py-8">
      <StepBar steps={DOC_STEPS.invitationLetter} className="mb-6" />
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="px-6 py-6 sm:px-8">
          ...
          <p className="mt-2 text-center text-sm text-gray-500">
            If you have an invitation letter from a host in {destination}, upload it here.
            This is optional but strengthens your application.{" "}
        ...
        <div className="border-t border-gray-100 px-6 py-4 sm:px-8 flex flex-col gap-3">
          ...
          >
            {allUploads.length > 0 ? "Finish" : "Skip & Finish"}
          </button>
        </div>
      </div>
      ...
      {showInfo && (
        <InfoModal title="Invitation Letter" onClose={() => setShowInfo(false)}>
          <p>If someone residing in the destination country has personally invited you, their invitation letter <strong>strengthens your application</strong> significantly.</p>
          <p className="mt-3">The letter should include the host&apos;s name, address, your relationship, purpose of visit, dates, and whether they are covering any expenses.</p>
          <p className="mt-3 text-blue-600 font-medium text-xs">This document is completely optional — skip if you have no host.</p>
        </InfoModal>
      )}
    </div>
  );
