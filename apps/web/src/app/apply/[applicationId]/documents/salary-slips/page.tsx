...
type MissingReason = "joined_recently" | "salary_pending" | "waiting_for_hr" | "on_leave";

const MISSING_REASONS: { value: MissingReason; label: string }[] = [
  { value: "joined_recently", label: "I joined recently" },
  { value: "salary_pending", label: "Salary pending" },
  { value: "waiting_for_hr", label: "Waiting for HR" },
...
type ViewMode = "upload" | "ocr_review";
...
export default function SalarySlipsPage() {
  ...
  const extractOcr = useAction(api.documents.extractSalarySlipData);
  const updateExtracted = useMutation(api.documents.updateExtractedData);
  const markUploaded = useMutation(api.applications.markDocumentsUploaded);
  ...
  async function handleUpload(index: number, file: File) {
    ...
    try {
      ...
      // Kick off OCR in background for first uploaded slip
      if (index === 0) {
        try {
          const extracted = await extractOcr({ documentId: docId });
          const e = extracted as Record<string, string | null>;
          setOcrData({
          ...
          // OCR failure is non-blocking — user can enter manually
        }
      }
    } catch {
  ...
  const allResolved = slots.every(s =>
    s.phase === "uploaded" ||
    (s.phase === "missing_reason" && s.confirmed),
  );

  const anyUploaded = slots.some(s => s.phase === "uploaded");

  function handleNextToOcr() {
    // Skip OCR review if no docs were actually uploaded
    if (!anyUploaded) {
      void markUploaded({ applicationId });
      router.push(`/apply/${applicationId}/documents/leave-letter`);
      return;
    }
    setViewMode("ocr_review");
  }

  async function handleConfirmOcr() {
    setSavingOcr(true);
    try {
      // Save OCR data to first uploaded slip
      const firstUploaded = slots.find(s => s.phase === "uploaded") as
        | Extract<SlotState, { phase: "uploaded" }>
        | undefined;

      if (firstUploaded) {
        await updateExtracted({
          documentId: firstUploaded.documentId,
          extractedData: ocrData,
        });
      }

      await markUploaded({ applicationId });
      router.push(`/apply/${applicationId}/documents/leave-letter`);
    } finally {
      setSavingOcr(false);
    }
  ...
  if (viewMode === "ocr_review") {
    return (
      <OcrReviewView
        uploadedCount={slots.filter(s => s.phase === "uploaded").length}
        ocrData={ocrData}
    ...
    <div className="mx-auto max-w-2xl px-4 py-4 sm:py-8">
      ...
      <div className="mb-4 flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
        ...
        <p className="text-xs text-blue-700">
          We recommend uploading salary slips for the last 3 months. This helps strengthen your visa application.
        </p>
      </div>

      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="px-6 py-6 sm:px-8">
          <h1 className="text-center text-xl font-bold text-gray-900 sm:text-2xl">Salary Slips</h1>
          <p className="mt-2 text-center text-sm text-gray-500">
            Please upload your salary slips.{" "}
            <button type="button" onClick={() => setShowInfo(true)} className="font-medium text-blue-600 hover:underline">What&apos;s this?</button>
          </p>
          ...
          {/* Verification badges — shown when all resolved and at least one uploaded */}
          {allResolved && anyUploaded && (
            <div className="mt-4 flex flex-col gap-2">
              <VerifyBadge
                label={`Last ${slots.filter(s => s.phase === "uploaded").length} Months' Dates Found`}
              ...
              />
              {(() => {
                const skippedIdx = slots.findIndex(s => s.phase === "missing_reason" && s.confirmed);
                ...
                return skipped && reasonLabel ? (
                  <p className="text-center text-xs text-gray-400">
                    Month {skippedIdx + 1} skipped because {reasonLabel.toLowerCase()}.
                  </p>
                ) : null;
              })()}
            </div>
      ...
      {showInfo && (
        <InfoModal title="Salary Slips" onClose={() => setShowInfo(false)}>
          <p>Salary slips (pay stubs) are monthly documents from your employer showing your gross salary, deductions, and net take-home pay.</p>
          <p className="mt-3">The embassy uses them to verify you have a <strong>stable, regular income</strong> and will return home after your trip. Upload your last 3 months for the strongest application.</p>
        </InfoModal>
      )}
    </div>
  );
...
function SlotCard({
  label, state, onUpload, onToggleMissing, onSelectReason, onConfirmReason, onGoBack,
}: SlotCardProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            <>
              ...
              <div className="flex items-center justify-between border-t border-gray-100 px-2 py-1.5">
                <div className="flex items-center gap-1">
                  <CheckCircleSmall className="h-3.5 w-3.5 text-green-500" />
                  <span className="text-[10px] font-medium text-green-600 sm:text-xs">Document uploaded</span>
                </div>
                <div className="flex gap-1">
                  <IconBtn aria-label="Zoom in"><ZoomInIcon /></IconBtn>
                  <IconBtn aria-label="Zoom out"><ZoomOutIcon /></IconBtn>
                  <IconBtn aria-label="Delete" onClick={onGoBack}><TrashIcon /></IconBtn>
                </div>
              </div>
            </>
          ) : (
        ...
        <div className={cn(
          ...
          {isConfirmedMissing ? (
            <div className="flex flex-col items-center gap-2 py-2 text-center">
              <DocSkippedIcon className="h-8 w-8 text-gray-300" />
              <span className="text-[10px] font-semibold text-gray-600 sm:text-xs">Document Skipped</span>
              <span className="text-[10px] text-gray-400 sm:text-xs">
                Reason: {MISSING_REASONS.find(r => r.value === state.reason)?.label}
              </span>
              <button
                type="button"
                onClick={onGoBack}
            ...
            <>
              ...
              <div className="mb-3 flex flex-wrap gap-2">
                {MISSING_REASONS.map(({ value, label }) => (
                  <button
                    key={value}
                    type="button"
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={cn(
                    "flex-1 rounded-lg py-2 text-xs font-semibold text-white",
                    isMissing && state.reason
      ...
      {/* "I don't have this document" toggle */}
      {!isUploaded && (
...
interface OcrReviewViewProps {
  ...
  ocrData: OcrData;
  setOcrData: (d: OcrData) => void;
  onConfirm: () => void;
  saving: boolean;
}

function OcrReviewView({ uploadedCount, ocrData, setOcrData, onConfirm, saving }: OcrReviewViewProps) {
  return (
    <div className="mx-auto max-w-2xl px-4 py-4 sm:py-8">
      <StepBar steps={DATA_STEPS.employmentDocuments} className="mb-6" />
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="px-6 py-6 sm:px-8">
          <h1 className="text-center text-xl font-bold text-gray-900 sm:text-2xl">Salary Slip Details</h1>
          <p className="mt-2 text-center text-sm text-gray-500">
            We&apos;ve extracted details from your salary slip. Please review and confirm.
          </p>

          {/* Month thumbnails */}
          <div className="mt-5 grid grid-cols-3 gap-3">
            {[...Array(uploadedCount)].map((_, i) => (
              <div key={i} className="flex flex-col gap-1">
                <p className="text-xs font-semibold text-gray-700">Month {i + 1}</p>
                <div className="overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
                  ...
                  <div className="flex items-center justify-end gap-1 border-t border-gray-100 px-2 py-1">
                    <IconBtn aria-label="Zoom in"><ZoomInIcon /></IconBtn>
                    <IconBtn aria-label="Zoom out"><ZoomOutIcon /></IconBtn>
                    <IconBtn aria-label="Delete"><TrashIcon /></IconBtn>
                  </div>
                </div>
              </div>
            ))}
            ...
            {(["companyName", "designation", "companyAddress"] as const).map(field => (
              <div key={field} className="relative">
                ...
                <div className="flex items-center rounded-xl border border-green-300 bg-green-50 px-3 py-2.5">
                  <input
                    value={ocrData[field]}
                    onChange={e => setOcrData({ ...ocrData, [field]: e.target.value })}
                    className="flex-1 bg-transparent text-sm text-gray-800 outline-none"
                  />
                  <CheckCircleSmall className="ml-2 h-4 w-4 shrink-0 text-green-500" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="border-t border-gray-100 px-6 py-4 sm:px-8">
          <button
            ...
            className={cn(
              "flex w-full items-center justify-center rounded-xl py-3 text-sm font-semibold text-white transition-colors",
              !saving ? "bg-blue-600 hover:bg-blue-700" : "cursor-not-allowed bg-blue-300",
            )}
          >
            {saving ? <span className="flex items-center gap-2"><Spinner />Saving…</span> : "Confirm"}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-xl animate-pulse px-4 py-8">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-8">
        <div className="mx-auto mb-3 h-6 w-36 rounded-lg bg-gray-200" />
        <div className="mt-6 space-y-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-24 rounded-xl bg-gray-100" />)}
        </div>
      </div>
    </div>
  );
...
function Spinner() {
  return <svg className="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12h4Z" /></svg>;
}
